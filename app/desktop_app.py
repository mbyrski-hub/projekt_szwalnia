...
main_queue = queue.Queue() # Kolejka dla logów z innych wątków (np. synchronizacji)
root = None # Główny obiekt Tkinter
...
def load_config():
    if os.path.exists(CONFIG_FILE):
        try:
            ...
            print(f"Błąd odczytu pliku {CONFIG_FILE}. Zwracam pustą konfigurację.")
            return {}
    return {}
...
# Funkcja logowania do pliku i opcjonalnie do kolejki GUI
def log_message(queue, message, color='black', level='info'):
    ...
    # Wysłanie do kolejki GUI, jeśli podana
    if queue:
        queue.put({'msg': f"[{timestamp}] {message}\n", 'color': color})

# --- NOWA KLASA LOG HANDLER DLA SERWERA DRUKU ---
class PrintLogHandler:
    """Obsługuje logowanie bezpośrednio z wątku Flask do widgetu Tkinter."""
    def __init__(self, text_widget, tk_root):
        self.text_widget = text_widget
        self.tk_root = tk_root

    def write(self, message, color='black', level='info'):
        if message.strip():
            ...
            # Bezpieczna aktualizacja widgetu Tkinter z innego wątku
            # Używamy lambda, aby przekazać argumenty do _insert_text
            self.tk_root.after(0, lambda msg=f"[{timestamp}] {message}\n", clr=color: self._insert_text(msg, clr))

    def _insert_text(self, message, color_tag):
        try:
            self.text_widget.configure(state='normal')
            self.text_widget.insert(tk.END, message, color_tag)
            self.text_widget.configure(state='disabled')
            self.text_widget.see(tk.END)
        except tk.TclError as e:
            # Może się zdarzyć, jeśli okno zostanie zamknięte podczas aktualizacji
            print(f"Błąd Tkinter w PrintLogHandler: {e}")

    def flush(self):
        pass # Nie jest potrzebne przy bezpośrednim zapisie do widgetu
# --- KONIEC NOWEJ KLASY ---
...
def test_sql_connection(queue, config_data):
    # Bez zmian, używa log_message z kolejką
...
def get_data_from_warehouse(queue, config_data, warehouse_symbol):
    ...
    query = """
        WITH LastPurchasePerSupplier AS (
            SELECT
                p.Asortyment_Id, d.PodmiotId, k.Wartosc, k.Ilosc, k.Data,
        ...
        SELECT
            a.Symbol, a.Nazwa, pod.NazwaSkrocona AS NazwaDostawcy,
            lpps.Data AS DataOstatniegoZakupu, (lpps.Wartosc / lpps.Ilosc) AS CenaJednostkowaNetto
...
def send_prices_to_webapp(queue, config_data, data_to_send):
    ...
    data_for_api = [
        {'symbol': item['symbol'], 'name': item.get('name'), 'price': item.get('price'),
         'supplier': item.get('supplier'), 'price_date': item.get('price_date')}
...
def report_ip_to_webapp(queue, config_data):
    ...
    if not current_ip or current_ip == '127.0.0.1':
        ...
        return
    url = f"{config_data.get('web_app_url')}/api/report_desktop_ip"
    headers = {'Content-Type': 'application/json', 'X-API-KEY': config_data.get('api_key')}
...
def create_shortcut_in_startup(target_path, shortcut_name):
    ...
    if not os.path.exists(target_path):
         messagebox.showwarning("Błąd", f"Nie można utworzyć skrótu, plik docelowy nie istnieje:\n{target_path}\n\nSprawdź po kompilacji.")
         return False
    try:
...
def remove_shortcut_from_startup(shortcut_name):
    ...
    if os.path.exists(shortcut_path):
        try: os.remove(shortcut_path); return True
        except Exception as e: messagebox.showerror("Błąd Autostartu", f"Nie udało się usunąć skrótu: {e}"); return False
    return False
...
def print_label():
    # === ZMIANA: Pobranie dedykowanego handlera dla serwera druku ===
    log_handler = flask_app_print.config.get('PRINT_LOG_HANDLER')
    if not log_handler:
         # Zapasowe logowanie do pliku, jeśli handler nie został poprawnie przekazany
         logger.error("BŁĄD KRYTYCZNY: Brak PrintLogHandler w konfiguracji Flask!")
         return jsonify({'error': 'Błąd konfiguracji serwera druku (brak log handlera).'}), 500
    # === KONIEC ZMIANY ===
    ...
    if not selected_printer or selected_printer == "Brak drukarek":
        log_handler.write("BŁĄD: Żądanie drukowania odrzucone - brak wybranej drukarki.", color='red', level='error') # Dodano level='error'
        return jsonify({'error': 'Drukarka nie została wybrana na serwerze druku.'}), 500

    if 'image' not in request.files:
        log_handler.write("BŁĄD: Żądanie drukowania odrzucone - brak pliku obrazu.", color='red', level='error') # Dodano level='error'
    ...
    try:
        ...
        log_handler.write("BŁĄD: Żądanie drukowania odrzucone - niepoprawne wymiary/ilość.", color='red', level='error') # Dodano level='error'
    ...
    try:
        ...
        try:
            ...
            for i in range(quantity):
                ...
                printer_dc.EndPage()
                printer_dc.EndDoc()
            log_handler.write("Wszystkie kopie wysłane pomyślnie przez sterownik!", color='green') # Dodano kolor
        finally:
            if 'printer_dc' in locals() and printer_dc: # Zamknij DC, jeśli został utworzony
                 printer_dc.DeleteDC()
            if 'h_printer' in locals() and h_printer: # Zamknij drukarkę, jeśli została otwarta
                 win32print.ClosePrinter(h_printer)

        return jsonify({'status': f'Wydrukowano pomyślnie {quantity} kopii.'})

    except Exception as e:
        # Zapisz pełny traceback do logu plikowego dla lepszej diagnostyki
        logger.exception("KRYTYCZNY BŁĄD PODCZAS DRUKOWANIA:")
        # Wyślij uproszczoną wiadomość do GUI
        log_handler.write(f"KRYTYCZNY BŁĄD PODCZAS DRUKOWANIA: {e}", color='red', level='error')
        return jsonify({'error': f'Błąd serwera druku: {str(e)}'}), 500
...
# Zmieniono, aby używać flask_app_sync zamiast flask_app_print do nasłuchiwania
@flask_app_sync.route('/trigger-sync', methods=['POST'])
def trigger_sync():
    # Logowanie przez kolejkę, bo to serwer synchronizacji
    log_message(main_queue, "Odebrano zdalne polecenie synchronizacji ze strony WWW.")
    ...
    if not api_key_from_request or api_key_from_request != current_config.get('api_key'):
        log_message(main_queue, "Odebrano próbę zdalnego uruchomienia synchronizacji z BŁĘDNYM kluczem API.", color='red', level='error')
        return jsonify({'error': 'Brak autoryzacji'}), 401

    sync_thread = threading.Thread(target=full_sync_task, args=(main_queue, current_config), daemon=True)
    sync_thread.start()
...
class PrintServerApp:
    def __init__(self, root_tk): # Zmieniono nazwę argumentu na root_tk
        global root # Użyj globalnej zmiennej root
        root = root_tk # Przypisz główny obiekt Tkinter do globalnej zmiennej
        self.root = root_tk # Zachowaj referencję lokalną
        self.root.title("Lokalny Serwer Drukowania i Synchronizacji")
        self.root.geometry("700x600")
        self.root.minsize(600, 500)
        self.tray_icon = None
        self.queue = main_queue # Użyj globalnej kolejki

        # Inicjalizacja widgetu logów
        log_frame = ttk.LabelFrame(self.root, text="Log operacji") # Dodano self.root
        log_frame.pack(fill='both', expand=True, padx=10, pady=10) # Dodano pady
        ...
        # === ZMIANA: Tworzenie dedykowanego handlera dla serwera druku ===
        self.print_log_handler = PrintLogHandler(self.main_log_text, self.root)
        flask_app_print.config['PRINT_LOG_HANDLER'] = self.print_log_handler # Przekaż go do Flaska
        # === KONIEC ZMIANY ===

        self.notebook = ttk.Notebook(self.root) # Dodano self.root
        self.print_tab = ttk.Frame(self.notebook)
        self.sync_tab = ttk.Frame(self.notebook)
        # Przeniesiono zakładkę logu na koniec
        self.notebook.add(self.print_tab, text="Serwer Druku")
        self.notebook.add(self.sync_tab, text="Synchronizator")
        self.notebook.pack(expand=True, fill="both", padx=10, pady=(0, 10)) # Usunięto log_tab, poprawiono pady

        # --- Zakładka Serwer Druku ---
        ...
        self.selected_printer = tk.StringVar(self.root) # Dodano self.root
        ...
        if saved_printer and saved_printer in self.available_printers:
            ...
            APP_CONFIG["SELECTED_PRINTER"] = saved_printer
            log_message(self.queue, f"Załadowano zapisaną drukarkę: {saved_printer}")
            ...
            log_message(self.queue, f"Brak zapisanej drukarki, wybrano domyślną: {self.available_printers[0]}")
        else:
            self.available_printers.append("Brak drukarek")
            self.selected_printer.set("Brak drukarek")
            APP_CONFIG["SELECTED_PRINTER"] = "Brak drukarek"
            log_message(self.queue, "Nie znaleziono żadnych drukarek.", color='orange')
        ...
        # --- Zakładka Synchronizator (bez zmian w definicji widgetów) ---
        ...
        self.warehouse_combo = ttk.Combobox(data_source_frame, textvariable=self.warehouse_var, state='readonly')
        ...
        # --- Inicjalizacja stanu UI i wątków ---
        self.root.after(100, self.process_queue) # Uruchom przetwarzanie kolejki logów
        self.update_full_sync_button_state()
        self.update_ip()
        self.start_sync_server() # Zawsze startuj serwer synchronizacji przy uruchomieniu
        self.root.protocol("WM_DELETE_WINDOW", self.minimize_to_tray)

        # Uruchomienie wątku harmonogramu
        threading.Thread(target=scheduler_thread_func, args=(self.queue, self.config), daemon=True).start()

        # --- Logika auto-startu serwera druku i minimalizacji ---
        if self.autostart_var.get():
            log_message(self.queue, "Wykryto ustawienie autostartu. Automatyczne uruchamianie serwera druku i minimalizacja...", color='blue')
            self.root.after(100, self.start_print_server)
            self.root.after(500, self.minimize_to_tray)

    def create_tray_image(self):
        # Bez zmian
        image = Image.new('RGB', (64, 64), 'gray'); dc = ImageDraw.Draw(image)
        dc.rectangle((10, 10, 54, 54), fill='white'); dc.text((22, 22), "S", fill="black"); return image
    ...
    def minimize_to_tray(self):
        # Bez zmian
        if self.tray_icon and self.tray_icon.visible: log_message(self.queue, "Aplikacja jest już zminimalizowana.", color='orange'); return
        self.root.withdraw(); image = self.create_tray_image()
        menu = (pystray.MenuItem('Pokaż', self.show_window, default=True), pystray.MenuItem('Zakończ', self.exit_app))
        self.tray_icon = pystray.Icon(APP_NAME, image, "Serwer Drukowania i Synchronizacji", menu)
        threading.Thread(target=self.tray_icon.run, daemon=True).start()
        log_message(self.queue, "Aplikacja zminimalizowana do zasobnika systemowego.")
    ...
    def on_closing(self, force=False):
        ...
        if force or messagebox.askokcancel("Zamknij", "Czy na pewno chcesz zamknąć serwer? To zatrzyma drukowanie i synchronizację."):
            ...
            global stop_scheduler_thread; stop_scheduler_thread.set()
            time.sleep(0.5); self.root.destroy(); os._exit(0)
    ...
    def on_printer_select(self, selected_value):
        ...
        if save_config(current_cfg): log_message(self.queue, "Wybór drukarki zapisany w konfiguracji.")
        else: log_message(self.queue, "Błąd zapisu wyboru drukarki.", color='red')

    def update_ip(self):
        # Bez zmian
        ip = get_local_ip(); self.ip_label.config(text=f"Twój adres IP w sieci lokalnej: {ip}"); log_message(self.queue, f"Sprawdzono IP: {ip}.")

    def start_print_server(self):
        # Bez zmian w logice, ale używa PrintLogHandler
        selected_printer = APP_CONFIG.get("SELECTED_PRINTER")
        if not selected_printer or selected_printer == "Brak drukarek":
            messagebox.showerror("Błąd", "Nie wybrano poprawnej drukarki! Sprawdź konfigurację.")
            log_message(self.queue, "Próba uruchomienia serwera druku bez wybranej drukarki.", color='red', level='error')
            return
        global flask_thread_print
        if flask_thread_print and flask_thread_print.is_alive(): log_message(self.queue, "Serwer druku już działa.", color='orange'); return
        log_message(self.queue, f"Uruchamianie serwera druku na {HOST}:{PORT_PRINT} dla drukarki: {selected_printer}...")
        def run_app():
            log = logging.getLogger('werkzeug'); log.disabled = True; flask_app_print.logger.disabled = True
            try: flask_app_print.run(host=HOST, port=PORT_PRINT, debug=False, use_reloader=False)
            except OSError as e:
                log_message(self.queue, f"BŁĄD KRYTYCZNY serwera druku: Port {PORT_PRINT} zajęty lub brak uprawnień. {e}", color='red', level='error')
                ...
                self.root.after(0, self.update_print_server_ui_on_error)
        flask_thread_print = threading.Thread(target=run_app, daemon=True); flask_thread_print.start()
        self.status_label_print.config(text="Status serwera druku: Uruchamianie...", fg="orange")
        self.start_button_print.config(state=tk.DISABLED); self.stop_button_print.config(state=tk.NORMAL); self.printer_menu.config(state=tk.DISABLED)
    ...
    def check_print_server_startup(self):
       ...
       else: self.update_print_server_ui_on_error()

    def update_print_server_ui_on_error(self):
        # Bez zmian
       self.status_label_print.config(text="Status serwera druku: Błąd startu", fg="red")
       self.start_button_print.config(state=tk.NORMAL); self.stop_button_print.config(state=tk.DISABLED)
       if self.available_printers and self.available_printers[0] != "Brak drukarek": self.printer_menu.config(state=tk.NORMAL)
       else: self.printer_menu.config(state=tk.DISABLED)

    def start_sync_server(self):
        # Bez zmian, używa log_message z kolejką
        global flask_thread_sync
        if flask_thread_sync and flask_thread_sync.is_alive(): log_message(self.queue, "Serwer synchronizacji już działa.", color='orange'); return
        log_message(self.queue, f"Uruchamianie serwera synchronizacji na {HOST}:{PORT_SYNC}...")
        def run_app():
            log = logging.getLogger('werkzeug'); log.disabled = True; flask_app_sync.logger.disabled = True
            try: flask_app_sync.run(host=HOST, port=PORT_SYNC, debug=False, use_reloader=False)
            except OSError as e:
                log_message(self.queue, f"BŁĄD KRYTYCZNY serwera sync: Port {PORT_SYNC} zajęty. {e}", color='red', level='error')
                self.root.after(0, self.update_sync_server_ui_on_error)
            except Exception as e:
                log_message(self.queue, f"Błąd krytyczny serwera sync: {e}", color='red', level='error')
                self.root.after(0, self.update_sync_server_ui_on_error)
        flask_thread_sync = threading.Thread(target=run_app, daemon=True); flask_thread_sync.start()
    ...
    def check_sync_server_startup(self):
       ...
       else: self.update_sync_server_ui_on_error()
    ...
    def stop_server(self):
        # Bez zmian
        log_message(self.queue, "Zatrzymywanie serwera...", color='orange'); self.on_closing(force=True)

    def toggle_startup(self):
        # Bez zmian
        is_enabled = self.autostart_var.get()
        if getattr(sys, 'frozen', False): target_path = sys.executable
        else: target_path = os.path.abspath(sys.argv[0])
        if is_enabled:
            if create_shortcut_in_startup(target_path, APP_NAME): log_message(self.queue, "Dodano aplikację do autostartu.")
            else: self.autostart_var.set(False)
        else:
            if remove_shortcut_from_startup(APP_NAME): log_message(self.queue, "Usunięto aplikację z autostartu.")

    def open_log_file(self):
        # Bez zmian
        try:
            log_path = os.path.abspath(log_file)
            if platform.system() == "Windows": os.startfile(log_path)
            elif platform.system() == "Darwin": subprocess.Popen(["open", log_path])
            else: subprocess.Popen(["xdg-open", log_path])
        except Exception as e: log_message(self.queue, f"Nie można otworzyć pliku logu '{log_path}': {e}", color='red', level='error')
    ...
    def get_current_config(self):
        ...
        return {
            ...
            'default_warehouse': self.warehouse_var.get(), 'autostart': self.autostart_var.get(), 'update_time': self.update_time_var.get(),
            'selected_printer': APP_CONFIG.get("SELECTED_PRINTER", self.selected_printer.get())
        }
    ...
    def save_current_config(self):
        ...
        if save_config(new_config):
            config = new_config; self.toggle_startup() # Wywołaj po zapisie
            messagebox.showinfo("Sukces", "Konfiguracja została zapisana."); self.update_full_sync_button_state()
            log_message(self.queue, "Restartowanie wątku harmonogramu...", color='gray')
            stop_scheduler_thread.set(); time.sleep(1.1); stop_scheduler_thread.clear()
            scheduler_thread_instance = threading.Thread(target=scheduler_thread_func, args=(self.queue, new_config), daemon=True)
            scheduler_thread_instance.start(); log_message(self.queue, "Wątek harmonogramu zrestartowany.", color='gray')

    def process_queue(self):
        # Przetwarza logi z kolejki main_queue (dla synchronizacji i innych wątków)
        try:
            while True:
             ...
             # Dodatkowe logowanie błędów przetwarzania kolejki
             print(f"Błąd w process_queue: {e}")
             logger.exception("Błąd w process_queue:")
    ...
    def run_full_sync(self):
        # Bez zmian
        if not self.config.get('default_warehouse'): messagebox.showwarning("Brak konfiguracji", "Skonfiguruj i zapisz domyślny magazyn."); return
    ...
    def run_load_warehouses(self):
        ...
        def task_wrapper():
            current_config_snapshot = self.get_current_config(); warehouses = get_warehouses_from_sql(self.queue, current_config_snapshot)
            if warehouses:
                def update_combo():
                    self.warehouse_combo['values'] = warehouses; self.warehouse_combo.config(state='readonly')
                ...
                self.root.after(0, lambda: self.warehouse_combo.config(state='disabled')); self.root.after(0, self.update_full_sync_button_state)
    ...
    def run_fetch_data(self):
        # Bez zmian
        if not self.warehouse_var.get(): messagebox.showwarning("Brak magazynu", "Najpierw wczytaj i wybierz magazyn."); return
        log_message(self.queue,"Pobieranie danych do podglądu...", color='blue')
        def task_wrapper():
            current_config_snapshot = self.get_current_config(); warehouse_symbol = self.warehouse_var.get().split(' ')[0]
            data = get_data_from_warehouse(self.queue, current_config_snapshot, warehouse_symbol)
            if data is not None: self.root.after(0, self.show_review_window, data)
        threading.Thread(target=task_wrapper, daemon=True).start()

    def show_review_window(self, data):
        # Bez zmian
        review_window = tk.Toplevel(self.root); review_window.title(f"Podgląd Danych ({len(data)} wpisów cenowych)"); review_window.geometry("950x500")
        review_window.transient(self.root); review_window.grab_set()
        cols = ('Symbol', 'Nazwa', 'Cena Netto', 'Data Ceny', 'Dostawca'); tree = ttk.Treeview(review_window, columns=cols, show='headings')
        ...
        vsb = ttk.Scrollbar(review_window, orient="vertical", command=tree.yview); tree.configure(yscrollcommand=vsb.set); vsb.pack(side='right', fill='y'); tree.pack(expand=True, fill='both', padx=10, pady=5)
        for item in data: tree.insert("", "end", values=(item['symbol'], item['name'], f"{item.get('price', 0.0):.2f} zł", item.get('price_date', 'Brak'), item.get('supplier', 'Brak')))
        def send_catalog_action(): review_window.destroy(); log_message(self.queue,"Ręczne wysyłanie katalogu...", color='blue'); threading.Thread(target=send_catalog_to_webapp, args=(self.queue, self.get_current_config(), data), daemon=True).start()
        def send_prices_action(): review_window.destroy(); log_message(self.queue,"Ręczne wysyłanie cen dostawców...", color='blue'); threading.Thread(target=send_prices_to_webapp, args=(self.queue, self.get_current_config(), data), daemon=True).start()
...
if __name__ == "__main__":
    root_tk = tk.Tk() # Użyj innej nazwy zmiennej
    app = PrintServerApp(root_tk)
    root_tk.mainloop()