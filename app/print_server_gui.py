...
from flask_cors import CORS # <-- Import dla komunikacji z przeglądarką
...
# Zmieniamy import na zgodny z nową wersją biblioteki
...
PRINTER_DPI = 203
LABEL_WIDTH_MM = 50
APP_NAME = "SerwerDrukuSzwalnia" # Nazwa dla skrótu w autostarcie
...
def get_startup_folder():
    """Zwraca ścieżkę do folderu Autostart dla bieżącego użytkownika."""
...
def create_shortcut_in_startup(target_path, shortcut_name):
    """Tworzy skrót do aplikacji w folderze Autostart."""
...
def remove_shortcut_from_startup(shortcut_name):
    """Usuwa skrót z folderu Autostart."""
    startup_path = get_startup_folder()
    shortcut_path = os.path.join(startup_path, f"{shortcut_name}.lnk")
...
# Włączamy CORS, aby serwer akceptował żądania z przeglądarki
CORS(flask_app)
...
def print_label():
    ...
    try:
        ...
        log_handler.write(f"Odebrano plik ({len(img_bytes)} bajtów)")
        print_width_dots = int((LABEL_WIDTH_MM / 25.4) * PRINTER_DPI)
        zpl_header = f"^XA\n^PW{print_width_dots}\n"
...
class PrintServerApp:
    ...
    # --- Funkcje dla zasobnika systemowego ---
    ...
    def minimize_to_tray(self):
        ...
        self.log_handler.write("Aplikacja zminimalizowana do zasobnika.")
    ...
    def toggle_startup(self):
        ...
        # Sprawdź, czy program jest uruchomiony jako skompilowany .exe
        if getattr(sys, 'frozen', False):
             target_path = sys.executable
        else:
            # Jeśli uruchamiamy jako skrypt, pokaż ostrzeżenie
